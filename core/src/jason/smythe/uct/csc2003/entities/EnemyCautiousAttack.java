...
public class EnemyCautiousAttack extends Enemy{

//	Player player;
	boolean useAlturnateTarget;
	
	public EnemyCautiousAttack(Sprite sprite, EnemyState enemyState, int startX, int startY, Player player, boolean b) {
		super(sprite, enemyState, startX, startY, player, b);
...
//		this.player = player;
	...
	public Vector2 drawPath(ShapeRenderer shapeRenderer, int i, boolean draw) {
		...
		if (useAlturnateTarget) {
			...
			shapeRenderer.circle(end.x, end.y, 3);
			shapeRenderer.circle(end.x, end.y, 5);
			shapeRenderer.circle(end.x, end.y, 7);
			shapeRenderer.circle(end.x, end.y, 9);
			shapeRenderer.circle(end.x, end.y, 11);
			shapeRenderer.circle(end.x, end.y, 13);
			shapeRenderer.circle(end.x, end.y, 15);
...
//	// a modified getTargetNode class, this 
	protected AStarNode getTargetNode() {
		if (playerState.position.dst(state.position) > 200) {
			...
			Square midPoint = 
					Square.getSquare(
							player.midPointPos.x + 
							Square.xOffset, 
							player.midPointPos.y + 
							Square.yOffset);
