...
//	private int counter;
	...
	public Enemy(Sprite sprite, EnemyState enemyState, int startX, int startY, Player player, boolean b) {
		...
		Gdx.app.log("square of enemy", state.curSquare.toString());
		state.position = state.curSquare.squareToPos();
		Gdx.app.log("position of enemy", state.position.toString());
	}
...
//		Gdx.app.log("", "Current State: " + state.moveState + ", next state: " + state.nextMoveState);
...
//		state.printState("Before: ");
		...
		if(state.path.length > 0) {
			state.moveState = state.path[0].movement;
		} else {
			state.moveState = Movement.STATIONARY;
		}
...
//	
//	public void updateStcalculatePathate() {
//		state.curSquare.transferValues(state.nextSquare);
//		state.position = state.nextPosition;
//		
//		state.setNextSquareAndPos(state.nextMoveState);
//		state.moveState = state.nextMoveState;
//	}
