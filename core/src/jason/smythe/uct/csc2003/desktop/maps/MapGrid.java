...
//				Square square = new Square(x, y);
				mapGrid[x][y] = new AStarNode(x, y, map.getSquareType(x, y));
//				System.out.println(mapGrid[x][y].square + ":" + mapGrid[x][y].squareType + "; ");
		...
		for (int x = 0;x < 17; x++ ) {
			for (int y = 0; y < 13; y++ ) {
				...
				if (mapGrid[x][y].squareType == SquareType.BLT 
					|| mapGrid[x][y].squareType == SquareType.BLTR) {
					mapGrid[x][y].upCan = true;
				}
					
				if ( (y > 0) 
						&& (mapGrid[x][y - 1].squareType == SquareType.BLT
							|| mapGrid[x][y - 1].squareType == SquareType.BLTR)) {
					mapGrid[x][y].downCan = true;
				}

				if ( (x > 0) 
						&& (mapGrid[x - 1][y].squareType == SquareType.BLR
							|| mapGrid[x - 1][y].squareType == SquareType.BLTR)) {
					mapGrid[x][y].leftCan = true;
				}

				if ( mapGrid[x][y].squareType == SquareType.BLR
						|| mapGrid[x][y].squareType == SquareType.BLTR) {
					mapGrid[x][y].rightCan = true;
				}
...
//			connectedNodes.
			AStarNode top = mapGrid[node.x][node.y + 1];
...
//				System.out.println("added top: " + top);
		...
		if (node.squareType == SquareType.BLR || node.squareType == SquareType.BLTR) {
			AStarNode right = mapGrid[node.x + 1][node.y];
...
//				System.out.println("added right: " + right);
		...
		if (node.y > 0 && 
				(mapGrid[node.x][node.y - 1].squareType == SquareType.BLT 
				|| mapGrid[node.x][node.y - 1].squareType == SquareType.BLTR) ) {
//			connectedNodes.
			AStarNode bottom = mapGrid[node.x][node.y - 1];
...
//				System.out.println("added bottom: " + bottom);
		...
		//left node available
		if (node.x > 0 && 
				(mapGrid[node.x - 1][node.y].squareType == SquareType.BLR 
				|| mapGrid[node.x - 1][node.y].squareType == SquareType.BLTR) ) {
			AStarNode left = mapGrid[node.x - 1][node.y];
...
//				System.out.println("added left: " + left);
	...
	public void printMapGrid(){
		for (int x = 0;x < 17; x++ ) {
			for (int y = 0; y < 13; y++ ) {
				System.out.print(mapGrid[x][y] + ":" + mapGrid[x][y].squareType + "; ");
			}
			System.out.println();
		}
	}
