...
public class PlayController {
	...
	public PlayController(GameSoundController sound, String mapToLoad, boolean specialDemo) {
		...
		renderer = new OrthogonalTiledMapRenderer(maps.currentMap.map, h / 650);

        camera = new OrthographicCamera();
        camera.setToOrtho(false, w, h); //add offsets for the grid here!
        camera.update();
        shapeRenderer = new ShapeRenderer();
        shapeRenderer.translate((float) 0.5*Square.squareSize, (float) 0.5*Square.squareSize, 0);
	...
	public void resetEnemies() {	
		if (specialDemo) {
			...
			enemies = new Enemy[4];
			enemies[0] = new Enemy(new Sprite(new Texture("img/microchip.png")), new EnemyState(maps), 16, 5, player, true);
			enemies[1] = new Enemy(new Sprite(new Texture("img/microchip.png")), new EnemyState(maps), 16, 7, player, false);
			enemies[2] = new Enemy(new Sprite(new Texture("img/microchip.png")), new EnemyState(maps), 16, 8, player,	false);
			enemies[3] = new Enemy(new Sprite(new Texture("img/microchip.png")), new EnemyState(maps), 16, 4, player, false);
		} else {
			//I pass in the PlayController and sound for the sake of the handling of the lose condition.
			enemies = new Enemy[4];
			enemies[0] = new Enemy(new Sprite(new Texture("img/microchip.png")), new EnemyState(maps), 0, 0, player, true);
			enemies[1] = new Enemy(new Sprite(new Texture("img/microchip.png")), new EnemyState(maps), 16, 0, player, false);
			enemies[2] = new EnemyGuard(new Sprite(new Texture("img/shield.png")), new EnemyState(maps), 16, 7, player, false);
			enemies[3] = new EnemyCautiousAttack(new Sprite(new Texture("img/crossEnemy.png")), new EnemyState(maps), 0, 7, player, false);
		}
	}
